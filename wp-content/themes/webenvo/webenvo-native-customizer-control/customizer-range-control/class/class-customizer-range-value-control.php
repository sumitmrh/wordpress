...
define( 'WEBENVO_RANGE_VERSION', '1.0.0' );

/**
 * Class Webenvo_Customizer_Range_Value_Control
 *
 * @access public
 */
class Webenvo_Customizer_Range_Value_Control extends WP_Customize_Control {
	...
	/**
	 * Hestia_Webenvo_Customizer_Range_Value_Control constructor.
	 *
	...
	public function enqueue() {
		wp_enqueue_script( 'customizer-range-value-control', get_template_directory_uri() . '/customizer-range-control/js/customizer-range-value-control.js', array( 'jquery', 'customize-base' ), WEBENVO_RANGE_VERSION, true );
		wp_enqueue_style( 'customizer-range-value-control', get_template_directory_uri() . '/customizer-range-control/css/customizer-range-value-control.css', array(), WEBENVO_RANGE_VERSION );
	}
	...
	public function json() {
		...
		$json['desktop_value'] = ! $this->webenvo_is_json( $json['value'] ) ? $json['value'] : $json['default_value'];
		$json['tablet_value']  = $json['default_value'];
		$json['mobile_value']  = $json['default_value'];

		if ( $this->webenvo_is_json( $json['value'] ) ) {
	 ...
	 */
	public function webenvo_is_json( $string ) {
 ...
 */
function webenvo_sanitize_range_value( $input ) {
	if ( webenvo_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function webenvo_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
