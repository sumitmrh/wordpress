...
// admin notice
(function ($) {
    var $notice_container = $('.hillstar-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('hillstar_disable_big_notice', {
            nonce: hillstar_builder_status.hillstar_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.hillstar-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.hillstar-admin-big-notice').addClass('processing');
            $('.hillstar-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.hillstar-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(hillstar_builder_status.messages.installing);
        $.get(hillstar_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.hillstar-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(hillstar_builder_status.messages.activating);

        wp.ajax.post('hillstar_activate_plugin', {
            slug: hillstar_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: hillstar_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.hillstar-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('hillstar_front_set_predesign', {
            index: selectedFrontPage,
            nonce: hillstar_builder_status.hillstar_front_set_predesign_nonce
        });
        if (hillstar_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (hillstar_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = hillstar_builder_status.view_demos_url;
        });
    });
    ...
    var hillstarInstallPluginSuccess = function hillstarInstallPluginSuccess(response) {
    ...
    var hillstarInstallPlugin = function hillstarInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: hillstarInstallPluginSuccess
        });
    };

    var hillstarActivatePlugin = function hillstarActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", hillstarInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", hillstarActivatePlugin);
